...
# modules
from salad.modules.protein_structure_diffusion import (
    Condition, assign_dssp, get_pair_condition,
...
class LatentDiffusion(hk.Module):
    ...
    def prepare_condition(self, data):
        ...
        dssp, _, block_adjacency = assign_dssp(
...
class Decoder(hk.Module):
    ...
    def __call__(self, data, predict_aa=False):
        ...
        def apply_model(data, prev):
            ...
            local, pos, condition, resi, chain, batch, mask, pair_condition = features
        ...
        # run recycling without gradients
        prev = jax.lax.stop_gradient(hk.fori_loop(0, count, iteration, prev))
        ...
        latent_update = MLP(
            local.shape[-1] * 2, data["latent_noised"].shape[-1],
            activation=jax.nn.gelu,
            bias=False,
            final_init=init_zeros())(local)
        new_latent = data["latent_noised"] + latent_update
        ...
        pair = MLP(
            ...
            final_init=init_glorot())(pair)
        # distogram predictions
        distogram = Linear(64, bias=False, initializer=init_zeros())(pair)
        # error predictions
        error = Linear(64, bias=False, initializer=init_zeros())(pair)
    ...
    def prepare_features(self, data, prev):
        ...
        return local, pos, condition, resi, chain, batch, mask, pair_condition
    ...
    def loss(self, data, result, err_result):
        ...
        # sigma2 = data["time"] ** 2
        # alpha2 = 1 - sigma2
        # weight = alpha2 / sigma2
        latent_loss = ((result["latent"] - data["latent"]) ** 2).mean(axis=-1)
        latent_loss = (latent_loss * base_weight).sum()
        losses["latent"] = latent_loss
        total += 10.0 * latent_loss
        ...
        # FAPE
        trajectory_fape, _, final_ae, _ = hal_atom_fape(
            Vec3Array.from_array(result["atom_trajectory"]),
        ...
        trajectory_fape_loss = 2 * trajectory_fape[-1] + trajectory_fape.mean()
        losses["trajectory_fape"] = trajectory_fape_loss
        total += 10.0 * trajectory_fape_loss
...
class AADecoder(hk.Module):
    ...
    def __call__(self, aa, local, pos, resi, chain, batch, mask,
        ...
        neighbours = extract_neighbours(num_index=0, num_spatial=32, num_random=0)(
            Vec3Array.from_array(pos), resi, chain, batch, mask)
        local = AADecoderStack(c, depth=2)(aa, local, pos, neighbours,
                                           resi, chain, batch, mask,
                                           priority=priority)
        local = hk.LayerNorm([-1], True, True)(local)
            ...
            def _inner(data):
                ...
                neighbours = extract_neighbours(
                    ...
                    num_random=c.random_neighbours)(
                        ...
                        resi, chain, batch, mask)
                if c.pair_condition:
                    neighbours = get_contact_neighbours(
                        c.condition_neighbours or 16)(
...
class DecoderBlock(hk.Module):
    ...
    def __call__(self, features, pos,
        ...
        if pair_condition is not None:
            index = jnp.arange(neighbours.shape[0], dtype=jnp.int32)
            pair += Linear(c.pair_size, bias=False)(pair_condition[index[:, None], neighbours])
        pair = hk.LayerNorm([-1], True, True)(pair)
...
class Condition(hk.Module):
    ...
    def __call__(self, aa, sse, pos, pos_mask, residue_index, chain_index, batch_index, do_condition=None):
        sequence_mask, sse_mask, structure_mask, sse_mean_mask = self.get_masks(
            chain_index, batch_index, do_condition=do_condition)

        # FIXME: adjust structure mask to stop leaking stuff left and right
        ...
        sse_latent = Linear(c.local_size, initializer=init_glorot())(jax.nn.one_hot(sse, 3, axis=-1))
        sse_latent = hk.LayerNorm([-1], True, True)(sse_latent)

        sse_mean = index_mean(jax.nn.one_hot(sse, 3, axis=-1), chain_index, pos_mask.any(axis=-1, keepdims=True))[:, 1:] # FIXME: was 0-1 (LH) now 1-2 (HE), which should be better
        sse_mean /= jnp.maximum(sse_mean.sum(axis=-1, keepdims=True), 1e-6)
        if (do_condition is not None) and (do_condition["sse_mean"] is not None):
            sse_mean = do_condition["sse_mean"]
        sse_mean = Linear(c.local_size, initializer=init_glorot())(sse_mean)
        sse_mean = hk.LayerNorm([-1], True, True)(sse_mean)
        ...
        condition = jnp.where(sequence_mask[..., None], aa_latent, 0) \
                  + jnp.where(sse_mask[..., None], sse_latent, 0) \
                  + jnp.where(structure_mask[..., None], bb_latent, 0) \
                  + jnp.where(sse_mean_mask[..., None], sse_mean, 0)
        return condition, sequence_mask

    def get_masks(self, chain_index, batch_index, do_condition=None):
        if do_condition is not None:
            sequence_mask = do_condition["aa"] != -1
            sse_mask = do_condition["sse"] != -1
            structure_mask = do_condition["structure"]["mask"]
            sse_mean_mask = jnp.ones_like(sse_mask) * (do_condition["sse_mean"] is not None)
