...
class InvariantStructureDiffusion(hk.Module):
    ...
    def prepare_data(self, data):
        ...
        data["mask"] = mask
        pos = self.encode(data)

        # set all-atom-position target
        atom_pos = pos_14
        atom_mask = atom_mask_14
        return dict(pos=pos, chain_index=chain, mask=mask,
    ...
    def apply_diffusion(self, data):
        ...
        relpos = frames[:, None, None].apply_inverse_to_point(pos[None, :])
        relpos = relpos
        t_pos = jax.random.uniform(
            hk.next_rng_key(), (batch.shape[0],))[batch]
        relpos_noised = diffuse_relpos_flow(
            hk.next_rng_key(), relpos, t_pos[:, None, None],
            scale=c.sigma_data)
        return dict(
            relpos_noised=relpos_noised.to_array(),
            t_pos=t_pos)
...
def diffuse_relpos_flow(key, relpos, time, scale=10.0) -> Vec3Array:
    noise = scale * jax.random.normal(key, list(relpos.shape) + [3])
    noise = Vec3Array.from_array(noise)
    return noise * time + relpos * (1 - time)

class InvariantStructureDiffusionInference(InvariantStructureDiffusion):
    def __init__(self, config,
                 name: Optional[str] = "structure_diffusion"):
...
class Diffusion(hk.Module):
    def __init__(self, config, name: Optional[str] = "diffusion"):
        super().__init__(name)
        self.config = config
        self.diffusion_block = InvariantDiffusionBlock
    ...
    def prepare_features(self, data):
        ...
        # edm_time_embedding = lambda x: log_sigma_embedding(
        #     x, size=64, min=jnp.log(0.001), max=jnp.log(120))
        # vp_time_embedding = lambda x: log_sigma_embedding(
        #     x, size=64, min=jnp.log(0.001), max=0.0)
...
class InvariantDiffusionBlock(hk.Module):
    def __init__(self, config, name: Optional[str] = "invariant_diffusion_block"):
        super().__init__(name)
        self.config = config

    def __call__(self, local, pos, relpos, resi, chain, batch, mask):
        c = self.config
        # TODO
        current_relpos = get_relpos(pos)
        current_neighbours = invariant_neighbours(current_relpos, resi, chain, batch, mask)
        initial_neighbours = invariant_neighbours(relpos, resi, chain, batch, mask)
        local += SparseRelposAttention(c)(hk.LayerNorm([-1], True, True)(local),
                                          current_relpos, resi, chain, batch, mask)
        local += SparseRelposAttention(c)(hk.LayerNorm([-1], True, True)(local),
                                          relpos, resi, chain, batch, mask)
        local += GlobalUpdate(c)(hk.LayerNorm([-1], True, True)(local), resi, chain, batch, mask)
        # TODO
        pos = ConcordantUpdate(c)(local, pos, resi, chain, batch, mask)
        return local, pos
