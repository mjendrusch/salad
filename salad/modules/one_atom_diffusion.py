# TODO TODO TODO
# implement diffusion model with a single atom per polymer residue
# and a single atom per SMOL atom.
# Take CA for protein residues, C1 for NA residues and the atom
# itself for any atom in SMOL or METAL residues.
# remove waters.
# use a non-equivariant diffusion architecture, augmented with
# directional and distance features between atoms.
# cap the network with an AA-type predictor + 2 layers of
# all-atom inference, which should attempt to place all amino acid
# and NA atoms from the positions of only the CA/C1 atoms.
# use VE/EDM noise.
# for conditioning, use the usual: distance map, block contacts, DSSP
# as well as non-standard features, such as partial residue information
# which gets encoded as separate atoms.
# this conditioning needs to be included at the dataset stage, as the
# diffusion model has to have fixed input size during training.
# alternatively, this type of conditioning can be added as input data
...
class StructureDiffusionInference(StructureDiffusion):
    ...
    def prepare_condition(self, data):
        ...
        pair_condition = dict(
            dmap=jnp.zeros_like(dmap),
            dmap_mask=jnp.zeros_like(dmap_mask),
