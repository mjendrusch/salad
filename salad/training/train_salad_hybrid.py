...
from salad.modules.experimental.salad_hybrid import HybridDiffusion
...
def model_step(config, rebatch=1, is_training=True):
    ...
    def step(data):
        ...
        res_dict["reconstruct"] = dict(marked=None, decoded=dict(
            residue_index=data["residue_index"][:1024],
            chain_index=data["chain_index"][:1024],
            batch_index=data["batch_index"][:1024],
            atom_pos=out["results"]["decoder"]["atom_pos"],
            pos=out["results"]["latent_pos"],
        ...
        if "denoised_decoder" in out["results"]:
            res_dict["denoise"] = dict(marked=None, decoded=dict(
                ...
                chain_index=data["chain_index"][:1024],
                batch_index=data["batch_index"][:1024],
                atom_pos=out["results"]["denoised_decoder"]["atom_pos"],
                pos=out["results"]["pos"],
                aatype=data["aa_gt"][:1024],
                atom_mask=data["all_atom_mask"][:1024]
            ))
...
def log_decoded(writer, full_name: str, value, step):
    ...
    if False:
        ...
        if step % 100 == 0:
            ...
            protein = Protein(
                np.array(atom14_to_atom37(value["pos"][:, :14], value["aatype"])),
                trp_type, # all tryptophan
                trp_mask,
                np.array(value["residue_index"]),
                np.array(value["batch_index"]),
                np.ones_like(trp_mask.astype(jnp.float32)))
            with open(f"{path}/{name}_latent_{step}.pdb", "wt") as f:
                f.write(to_pdb(protein))
...
if __name__ == "__main__":
    ...
    if opt.dataset == "SYNTHETE":
        ...
        num_aa = 1024
    path = opt.path
    path = f"{path}/salad_hybrid/hybrid-{opt.config}-{opt.num_aa}-{opt.suffix}"
