...
from salad.modules.experimental.sequence_design import AllAtomPotts
from salad.modules.config import sequence_design as config_choices
...
def make_valid_inner(step, data, multigpu=True, with_state=False):
    ...
    def valid_inner(loop_state: State):
        ...
        #params = loop_state.aux_state["ema_params"]
        res_dict = step(
...
if __name__ == "__main__":
    ...
    if opt.nanhunt == "True":
        ...
        NUM_DEVICES = 1
    path = opt.path
    path = f"{path}/sequence_design/potts-{opt.config}-{opt.num_aa}-{opt.suffix}"
    writer = SummaryWriter(path)

    print("Attempting to load dataset...")
    data = BatchedProteinPDBStream(f"{opt.data_path}/allpdb/",
                                   seqres_aa="clusterSeqresAA",
                                   cutoff_resolution=9.0,
                                   p_complex=opt.p_complex,
                                   size=opt.num_aa,
                                   min_size=16,
                                   max_size=opt.num_aa,
                                   legacy_repetitive_chains=False,
                                   split_across_workers=True)
    data = iter(BatchStream(data, num_workers=64,
