...
class AccumulateWrapper(Dataset):
  ...
  def next(self) -> Any:
    ...
    if "train" in items[0]:
      ...
      return {
        "valid": {
          name: np.concatenate(list(map(lambda x: x["valid"][name], items)), axis=0)
          for name in items[0]["valid"]
        }
      }
...
if __name__ == "__main__":
    ...
    path = f"{opt.path}/salad/nuadm-allpdb-{opt.config}-{opt.num_aa}" \
           f"-{opt.lr}-{opt.suffix}" 
    ...
    valid_data: BatchDistribution = AccumulateWrapper(BatchDistribution(
        valid_tasks,
        persistent_workers=True,
        prefetch_factor=8,
        num_workers=8
    ...
    train.train(params, opt_state, rng, data,
                valid=valid_data, aux_state=aux_state)
