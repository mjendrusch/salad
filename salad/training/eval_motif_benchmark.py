...
if __name__ == "__main__":
    ...
    with open(f"{opt.out_path}/constrings.jsonl", "wt") as cons_f:
        for idx in range(opt.num_designs):
            ...
            # np.savez("dmap.npy", dmap=dmap, dmap_mask=dmap_mask)
            data["dmap"] = dmap
            data["omap"] = omap
            # NOTE: DSSP conditioning is an issue
            # data["dssp_condition"] = dssp
            data["dmap_mask"] = dmap_mask
            # NOTE: removing aa conditioning hurts design
