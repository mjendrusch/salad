...
from salad.modules.noise_schedule_benchmark import (
    ...
    sigma_scale_cosine, sigma_scale_framediff,
...
def model_step(config):
    ...
    def step(data, prev):
        ...
        pos = data["pos"]
        pos = MotifAdjust().update_pos(pos, data["motifs"])
        data["pos"] = pos
        _, pos = predict.encode(data)
        data["pos"] = pos
...
def parse_template(path):
    ...
    def template_sampler(diversify=False):
        ...
        for conmask, consubset in conmasks:
            ...
            motifs.append((ncacocb, conmask))
        mask = np.zeros((num_aa,), dtype=np.bool_)
        mask[:true_length] = True
        return motifs, aatype, mask, constrings
...
if __name__ == "__main__":
    ...
    with open(f"{opt.out_path}/constrings.jsonl", "wt") as cons_f:
        for idx in range(opt.num_designs):
            motifs, aatype, mask, constrings = template_sampler(
                diversify=opt.diversify == "True")
            # write constraint dictionary to json list
