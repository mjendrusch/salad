...
components = gemmi.cif.read("components.cif.gz")
...
def parse_structure(path, max_chains=50):
            ...
            for subchain in chain.subchains():
                for res in subchain:
                    ...
                    for aoi, atom_name in enumerate(atom_order):
                        ...
                        entity_index.append(res.entity_id)
...
if __name__ == "__main__":
    ...
    import filelock
    import sys
    lock = filelock.FileLock("_asmcif.lock")

    base_name = "assembly"
    if len(sys.argv) > 1:
        base_name = sys.argv[1]

    for subdir in os.listdir(f"{base_name}_cif/"):
        subdir_list = os.listdir(f"{base_name}_cif/{subdir}/")
        while subdir_list: 
            with lock:
                print("locked")
                while subdir_list:
                    name = subdir_list.pop()
                    basename = name.split(".")[0]
                    if not (os.path.isfile(f"{base_name}_v3_npz/{subdir}/{basename}.npz") or os.path.isfile(f"wip_v3_{basename}")):
                        with open(f"wip_v3_{basename}", "w") as f:
                            f.write("wip")
                        break
                else:
                    continue
            print("unlocked")
            print(name)
            parse_result = parse_structure(f"{base_name}_cif/{subdir}/{name}")
            os.makedirs(f"{base_name}_v3_npz/{subdir}/", exist_ok=True)
            np.savez_compressed(f"{base_name}_v3_npz/{subdir}/{basename}.npz", **parse_result)
            os.remove(f"wip_v3_{basename}")
